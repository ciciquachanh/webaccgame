...
<style>
    .slider-wrapper {
        max-width: 1200px;
        margin: 80px auto 0 auto; /* ✅ Đẩy slider xuống khỏi phần trên */
        overflow: hidden;
        padding: 0 15px;
    }
    ...
    .slider-image {
        ...
        height: 300px;
        display: block;
        object-fit: fill; /* ✅ Không cắt ảnh, không viền đen */
    }
            ...
            <div id="slider" class="owl-theme section section-cate slideshow_full_width">
                <div id="slide_banner" class="owl-carousel">
                    @foreach($slider as $key => $sli)
                        <div class="item">
                            <a href="#" title="{{ $sli->title }}">
                                <img
                                    src="{{ asset('uploads/slider/' . $sli->image) }}"
                                    alt="{{ $sli->title }}"
                                    class="slider-image"
                                    onerror="this.src='https://via.placeholder.com/1200x300?text=No+Image';"
                                >
                            </a>
                        </div>
                    @endforeach
